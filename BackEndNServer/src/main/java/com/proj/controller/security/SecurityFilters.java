 ...
 * <p>
 * The Security Filter Chain is a chain of filters that intercepts all incoming
 * requests which are then processed through the filters.
 * <p>
 * The applied filters determine what action should be taken with the incoming
 * request, e.g. denied or require authentication (login).
 * 
...
public class SecurityFilters {
	...
	// We have disabled it as trying to implement it took longer than we had
	// allocated time for.
	// As per our report, we did not prioritise security features highly, hence why
	// we decided against spending more time to implement it.
	// To use CSRF protection, the client must send a CSRF token with POST/PUT
	// requests which the server recognises.
	// See:
	...
	// See:
	// https://docs.spring.io/spring-security/reference/servlet/authorization/method-security.html#method-security-architecture

	// ###############
	// #### Notes ####
	// ###############
	// @Order(1) -> The order in which to invoke this filter. No order defaults to
	// last.
	// TODO: Find a way to apply parts of the filters globally to reduce duplicate
	// code. E.g.: http.formlogin(...) or http.logout(...)
	//
	// Howto: Redirect to requested page upon successful login or prevent it:
	 ...
	 * and urls which are always public, e.g. "/".
	 * 
	 * @param http
	 * @return
	 * @throws Exception
	...
	public SecurityFilterChain allowFilter(HttpSecurity http) throws Exception {
		http
		.securityMatcher("/css/**", "/login/**", "/calendar", "/conmar", "/profile/**", "/about", "/contact", "/signup/**", "/", "/api/**", "/js/**", "/images/**", "/favicon.ico", "/error") /* All Http requests matching these paths will trigger this filter */
		.csrf((csrf) -> csrf.disable())
		.authorizeHttpRequests((authorize) -> authorize /* All requests most be authorized */
			.anyRequest().permitAll() /* Allow anything */
		);
	...
	/**
	 * Denies all requests matching this filter. This is useful for protecting
	 * private resources, e.g. resources used in development.
	 * 
	 * @param http
	 * @return
	 * @throws Exception
	...
	/**
	 * The basic security filter is a default/fallback authentication filter which
	 * requires authentication for all requests
	 * which are not specified in the requestMatcher methods. This is a basic
	 * security measure to prevent unathorized access in case someone
	 ...
	 * The actual authentication of users is handled by AuthenticationManager
	 * (roughly speaking).
	 * 
	 * @param http
	 * @return
	...
	/**
	 * Requires all requests to URLs specified in securityMatcher to require a role
	 * of admin+.
	 * <p>
	 * The actual authentication of users is handled by the AuthenticationManager
	 * (roughly speaking).
	 * 
	 * @param http
	 * @return
