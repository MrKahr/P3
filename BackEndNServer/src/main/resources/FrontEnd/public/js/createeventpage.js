...
createEventButton.addEventListener('click', () => {
  ...
  if (title == null || title == ""){alert("please add a valid title (max 40 characters) to your event, this can be changed later")};
  ...
  if (date == null || date == ""){alert("please add a valid date to your event, this can be changed later")};
  const maxNumberOfPlayers = document.querySelector('#eventMaxNumberOfPlayers').value;
  if (maxNumberOfPlayers == null || maxNumberOfPlayers == "" || maxNumberOfPlayers > 7){alert("please add valid a player maximum (max 7) to your event, this can be changed later")};
  
  if (title != null && location != null && description != null && date != null && maxNumberOfPlayers != null && title != "" && location != "" && description != "" && date != "" && maxNumberOfPlayers != "" && maxNumberOfPlayers <= 7) {
    try {
